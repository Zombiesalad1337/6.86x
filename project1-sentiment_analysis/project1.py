...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    #     loss += max(0, 1 - labels[k]*(np.dot(theta, feature_matrix[k,:]) + theta_0))
...
def perceptron_single_step_update(
    ...
    y = label * (feature_vector @ current_theta + current_theta_0)
    if y > eps:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            sum_theta_0 += theta_0
            count += 1
        # print(sum_theta / count,  sum_theta_0 / count)
    # print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
...
def pegasos_single_step_update(
    ...
    if (label * (current_theta @ feature_vector + current_theta_0)) <= 1 + 1e-7:
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(feature_matrix.shape[0]):
        if theta @ feature_matrix[i,:] + theta_0 > 1e-7:
            predictions[i] = 1
        else:
            predictions[i] = -1
    return predictions
